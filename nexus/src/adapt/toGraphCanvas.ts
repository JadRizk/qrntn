...
// (src/main.tsx) sets `theme="qrntn-hud"`, so the three ramp greys below
// are THAT variant's resolved values specifically — not "qrntn"'s AA ones,
// which are several steps lighter and would turn the etched substrate into
// visible lines.
//
// `hot` used to be documented as `--nx-fg-warning`. Under qrntn it is not:
...
const EDGE_HEX: Record<EdgeColorRole, string> = {
  hot: '#FF8A1E', // --nx-sodium, a content hue under qrntn — mid-run invocation reads as "active", not "critical" (--nx-alarm is reserved for refused/ghost)
  ...
  affirmed: '#87806C', // --nx-grey-600 ('qrntn-hud'), i.e. --nx-fg-muted — a verdict that landed: the lighter of the two greys, so `adopted` and `considered` stop being the same colour
  neutral: '#736D5C', // --nx-grey-500 ('qrntn-hud'), i.e. --nx-fg-subtle — a verdict that didn't, reading exactly as dim as the UI's own de-emphasized text
  etch: '#4C483D', // --nx-grey-300 ('qrntn-hud'), i.e. --nx-fg-disabled — the etched substrate, which you read as shape rather than as lines
}
