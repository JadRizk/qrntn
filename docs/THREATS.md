...
| `view` | The graph, in a browser at `localhost:7768` | Same: the CLI prints a URL and counts, never the graph. The browser renders held skills' **full artefact text** — as source, with every concealment character escaped and named (`nexus/src/reading/tokenise.ts`, keyed to this scanner's own classes; syntax colour is laid over that as ranges from a parser that emits a tree and never markup, `reading/highlight.ts`), into a page on this machine under a `Content-Security-Policy` that admits no remote source (`commands/view.mjs`, mirrored in `nexus/index.html`), and only for skills a human already adopted. Links in the text resolve to nodes in the same graph or to nothing; no link navigates off the page. **The port is fixed, so the server checks `Host`**: a request that does not name this machine — `localhost`, `127.0.0.1`, `[::1]`, at the bound port — is answered 403 and nothing else, which is what stops a page on another origin whose DNS is rebound to 127.0.0.1 from reading the graph as if same-origin (the attack behind Vite's CVE-2025-24010; the CSP does not defend against it, because it is not our page making the request). The browser is opened by the platform's own opener, handed the loopback URL and nothing else. See [`READING.md`](READING.md) and [`SERVING.md`](SERVING.md) |
