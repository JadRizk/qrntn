...
// What may legitimately sit at the root of the installed package, derived for
// the same reason the name above is. This was written down by hand until
// `files` gained CHANGELOG.md, at which point a shipped file was reported as
// something the tool had written at runtime — the assertion did not weaken, it
// pointed at the wrong thing, which is worse. `files` entries are paths; what
// lands at the root is their first segment. npm adds package.json whatever the
// manifest says.
...
const strayInPackage = readdirSync(pkgDir).filter((n) => !SHIPPED_AT_ROOT.has(n))
