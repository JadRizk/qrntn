...
//
// The verb is the contract; the filename behind it is not. bin/qrntn.mjs says
...
// `qrntn promote` was answered with a usage line for `promote.mjs` — a script
// that is not on their PATH, in a directory they have no reason to know about.
//
// The fix cannot be to hardcode `qrntn` either. These scripts are still run
...
// The name bin/qrntn.mjs sets when it spawns. Nothing else sets it.
export const VERB_ENV = 'QRNTN_VERB'
...
// verb — the same shape bin/qrntn.mjs's own table uses — instead of whatever
// happens to be in the variable.
const SAFE_VERB = /^[a-z][a-z-]{0,31}$/

/**
 * `qrntn promote` when the front door dispatched this, `node promote.mjs`
...
export function invokedAs(importMetaUrl) {
	...
	if (verb && SAFE_VERB.test(verb)) return `qrntn ${verb}`
